...
def query_vizier(radius, target_ra, target_dec,
                 catalog='gaia', max_stars=100, return_all_columns=False, timeout=5):
    ...
    if catalog == 'usno':
        ...
        catalog_id = 'J/ApJS/215/9/table5'
        out = 'RAJ2000,DEJ2000,F475W'
        sort='-F475W'
    ...
    URL = (orb.constants.VIZIER_URL + "asu-tsv/?-source=" + catalog_id
           + "&-c.ra=%f"%target_ra + '&-c.dec=%f'%target_dec
           + "&-c.rm=%d"%int(radius)
           + '&-out.max=%d'%int(max_stars*2)
           + '&-out.meta=-huD'
           + '&-out={}&-sort={}'.format(out, sort))

    logging.debug("URL sent : %s"%URL)

    retry = 0
    while retry <= MAX_RETRY:
        try:
            query_result = urllib2.urlopen(URL, timeout=timeout)
            break
    ...
    for iline in output:
        if iline[0] != '#' and iline[0] != '-' and len(iline) > 3:
            ...
            if len(iline) == params_number:
                if catalog == 'phat':
                    star_list.append(map(float, iline))
                else:
